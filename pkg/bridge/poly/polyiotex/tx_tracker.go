...
func (self *TransactionTracker) Start() error {
	...
	for {
		...
		lastCheckedBlockNo, _ := self.store.LastCheckedBlockNo(typ.NetPolygon, typ.NetIoTeX)
		...
		if err != nil {
			...
			err = self.store.UpdateLastCheckedBlockNo(toBlockNo, typ.NetPolygon, typ.NetIoTeX)
			if err != nil {
